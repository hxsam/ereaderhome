...
public class PlayerActivity extends Activity {
    ...
    private void restartSeekBarTimer() {
        ...
        mSeekbarTimer.scheduleAtFixedRate(new UpdateSeekbarTask(), SEEKBAR_DELAY_TIME, SEEKBAR_INTERVAL_TIME);
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (delta < MIN_SCRUB_TIME) delta = MIN_SCRUB_TIME;
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
                currentPos = mVideoView.getCurrentPosition();
                currentPos -= delta;
                if (currentPos > 0) play(currentPos);
                return true;
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                currentPos = mVideoView.getCurrentPosition();
                currentPos += delta;
                if (currentPos < mDuration) play(currentPos);
                return true;
    ...
    private void updatePlayButton(PlaybackState state) {
        switch (state) {
            case PLAYING:
                ...
                mPlayPause.setImageDrawable(getResources().getDrawable(R.drawable.ic_pause_playcontrol_normal));
                break;
            case PAUSED:
            case IDLE:
                mLoading.setVisibility(View.INVISIBLE);
                mPlayPause.setVisibility(View.VISIBLE);
                mPlayPause.setImageDrawable(getResources().getDrawable(R.drawable.ic_play_playcontrol_normal));
                break;
            case BUFFERING:
